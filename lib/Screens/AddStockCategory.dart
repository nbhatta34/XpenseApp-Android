...
class AddStockCategory extends StatefulWidget {
  const AddStockCategory({Key? key}) : super(key: key);

  @override
  State<AddStockCategory> createState() => _AddStockCategoryState();
}
...
class _AddStockCategoryState extends State<AddStockCategory> {
  ...
  Future<bool> addStockCategory(Category category) {
    var res = HttpConnectUser().addStockCategory(category, _image);
    return res;
  }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        child: Scaffold(
      appBar: AppBar(title: Text("Add Stocks Category")),
      body: Form(
        key: _formKey,
        child: Column(
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                image: DecorationImage(
                  fit: BoxFit.cover,
                  image: _image == null
                      ? NetworkImage("http://10.0.2.2:3000/uploads/upload.png")
                          as ImageProvider
                      : FileImage(_image!),
                ),
              ),
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      bool isCreated = await addStockCategory(category);
                      if (isCreated) {
